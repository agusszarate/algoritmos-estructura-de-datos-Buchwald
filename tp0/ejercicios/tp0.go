...
func sumaRecursiva(vector []int, index int) int {

	if index >= len(vector) {
		return 0
	}

	return vector[index] + sumaRecursiva(vector, index+1)
}
...
func EsCadenaCapicua(cadena string) bool {
	return esCapicuaRecursiva(cadena, 0, len(cadena)-1)
}

// esCapicuaRecursiva es la función auxiliar recursiva
func esCapicuaRecursiva(cadena string, inicio int, fin int) bool {
	...
	return esCapicuaRecursiva(cadena, inicio+1, fin-1)
}
